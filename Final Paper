...
Objectives
 ...
 To develop a functional Medical Records and Appointment System
...
Ensures secure and role-based access to information
...
The team has limited time and resources for the project. Information of the staff and patients will be provided by the hospital to ensure full consent and not violate any privacy or policy. 
...
The team needs to implement a proper database to store patient appointments without any clashes in dates and time that are already taken. Furthermore, if the operation is 
It is noted that the whole program is created using the Python language. Also note that data for patient records and login information for all users is stored in dictionaries.
